  ...
  useEffect(() => {
    ...
    const fetchUserProfile = async () => {
      try {
        ...
        const userId = Array.isArray(JSON.parse(userIdArray))
          ? parseInt(JSON.parse(userIdArray)[0], 10)
          : 4;
        const response = await axios.get(
          `https://vicious-damara-gentaproject-0a193137.koyeb.app/userprofile/${userId}`,
          {
            headers: {
        ...
        const allAddresses =
          data.addresses
            .map((addressObj: { address: string }) => addressObj.address)
    ...
    const fetchDiscounts = async () => {
      try {
        ...
        if (role === "seller") {
          const response = await axios.get(
            "https://vicious-damara-gentaproject-0a193137.koyeb.app/discounts"
          );
    ...
    const fetchProducts = async () => {
      try {
        const response = await axios.get(
          "https://vicious-damara-gentaproject-0a193137.koyeb.app/product"
        );
  ...
  useEffect(() => {
    ...
    if (role === "seller" && userIdArray) {
      ...
      const filtered = discounts.filter(
        (discount: any) => discount.seller_id === userId
      );
      ...
      <div className="container max-w-full mx-auto p-4 pt-[100px]">
        <div
          className={`flex flex-col md:flex-row items-center text-black ${
            isDarkMode
              ? "bg-[#888888] border-white"
              : "bg-white border-black"
          } p-6 border-2 rounded-lg overflow-hidden`}
        >
          ...
          <div className="flex-grow p-6">
            ...
            <div className="flex space-x-4">
              <button
                ...
                className="bg-[#40b446] text-white py-2 px-4 rounded-lg hover:bg-green-600"
              >
                Add Address
              </button>
              <button
        ...
        <div className="container max-w-full mx-auto p-4 pt-[10px]">
          {localStorage.getItem("userRole") === "seller" && (
            <div
              className={`border rounded pt-[10px] text-black ${
              isDarkMode
                ? "bg-[#888888] border-white"
                : "bg-white border-black"
              }`}
            >
              ...
              {filteredDiscounts.length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-4">
                  {filteredDiscounts
                   .filter((discount: any) => new Date(discount.expiration_date) > new Date())
                  .map((discount: any) => (
                    <div
                      key={discount.id}
                          ...
                          <ul>
                            {transaction.order_products.map((product, idx) => (
                              <li key={idx}>
                                ...
                                (Seller: {product.seller}) - {product.sum_price}
                              </li>
                            ))}
                          </ul>
                        </td>
